...
def apply_rabbit_hat_filter(image, face_landmarks, filter_image):

    image_height, image_width, _ = image.shape

    rabbit_hat_filter = cv2.imread("C:/Users/PC/image/hat3.png", cv2.IMREAD_UNCHANGED)
    ...
    # 모자 너비와 높이를 적절히 설정 (옆으로 더 넓게, 아래로 더 내려오게 조정)
    hat_height = int(image_height * 0.25)  # 얼굴의 25% 높이로 모자 설정
    hat_width = int(rabbit_hat_filter.shape[1] * (hat_height / rabbit_hat_filter.shape[0]) * 1.5)  # 원래 비율에 맞게 모자 너비 계산, 1.3배 넓게

    # 모자를 이마 위쪽으로 배치 (조금 더 내려오도록 조정)
    resize_and_apply_filter(rabbit_hat_filter, forehead_x - hat_width // 2 + 10, max(0, int(forehead_y - hat_height // 1.1)), hat_width, hat_height, image)
