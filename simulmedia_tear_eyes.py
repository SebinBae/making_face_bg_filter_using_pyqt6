...
def apply_tear_eyes_filter(image, face_landmarks):
    ...
    glasses_height = int(big_eye_filter.shape[0] * (glasses_width / big_eye_filter.shape[1]))  # 원래 비율에 맞춰 높이 계산

    # 안경 위치를 약간 위쪽으로 조정하여 자연스럽게 적용
    x_pos = left_eye_outer_x - int(glasses_width * 0.17)
    y_pos = int(middle_eye_y - glasses_height // 2.2)

    resize_and_apply_filter(big_eye_filter, x_pos, y_pos, glasses_width, glasses_height, image)
