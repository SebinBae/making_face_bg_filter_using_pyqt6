...
def apply_nose_dog_filter(image, face_landmarks):

    image_height, image_width, _ = image.shape

    nose_dog_filter = cv2.imread("C:/Users/PC/image/nose_dog.png", cv2.IMREAD_UNCHANGED)
    ...
    # 코의 가로, 세로 크기를 계산
    nose_width = abs(nose_right_x - nose_left_x) * 10  # 가로 크기를 10배 증가
    nose_height = abs(nose_bottom_y - nose_top_y) * 4  # 세로 크기를 4배 증가

    # 필터 중앙이 코 중앙에 오도록 조정
    x_pos = int((nose_left_x + nose_right_x) / 2 - nose_width / 2 - nose_width * 0.02)
    y_pos = int((nose_top_y + nose_bottom_y) / 2 - nose_height / 2 + nose_height * 0.1)

    # 필터 크기 조정
    resized_filter = cv2.resize(nose_dog_filter, (int(nose_width), int(nose_height)))
