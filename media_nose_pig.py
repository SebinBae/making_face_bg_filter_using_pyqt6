...
def apply_nose_pig_filter(image, face_landmarks):

    image_height, image_width, _ = image.shape

    nose_filter = cv2.imread("C:/Users/PC/image/nose_pig.png", cv2.IMREAD_UNCHANGED)
    ...
    # 코의 가로, 세로 크기를 계산
    nose_width = abs(nose_right_x - nose_left_x) * 3.7  # 가로 크기를 3.7배 증가
    nose_height = abs(nose_bottom_y - nose_top_y) * 1.3  # 세로 크기를 1.3배 증가

    # 필터 중앙이 코 중앙에 오도록 조정
    x_pos = int((nose_left_x + nose_right_x) / 2 - nose_width / 2 - nose_width * 0.05)
    y_pos = int((nose_top_y + nose_bottom_y) / 2 - nose_height / 2 + nose_height * 0.2)
