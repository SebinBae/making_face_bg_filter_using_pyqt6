...
def apply_mouth_lips_filter(image, face_landmarks):

    image_height, image_width, _ = image.shape

    mouth_filter = cv2.imread("C:/Users/PC/image/mouth_lips.png", cv2.IMREAD_UNCHANGED)
    ...
    # 필터 크기 계산
    mouth_width = mouth_right_x - mouth_left_x + 1
    if mouth_width > 0:
        mouth_height = int(mouth_filter.shape[0] * (mouth_width / mouth_filter.shape[1]))
        mouth_x = mouth_left_x - 5  # 살짝 왼쪽으로 이동
        mouth_y = mouth_left_y - mouth_height // 2 + 1

        # 필터 크기 조정
        resized_filter = cv2.resize(mouth_filter, (int(mouth_width * 1.3), int(mouth_height * 1.2)))
        ...
        # 필터 적용
        apply_rotated_filter(rotated_filter, mouth_x, mouth_y, int(mouth_width * 1.3), int(mouth_height * 1.2), image)
