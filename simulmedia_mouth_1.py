...
def apply_mouth_filter(image, face_landmarks, filter_image):
    ...
    red_thick_mouth_filter = cv2.imread("C:/Users/PC/image/mouth1.png", cv2.IMREAD_UNCHANGED)
    ...
    if mouth_width > 0:
        mouth_height = int(red_thick_mouth_filter.shape[0] * (mouth_width / red_thick_mouth_filter.shape[1]))
        mouth_x = mouth_left_x - 2  # 살짝 왼쪽으로 이동
        mouth_y = mouth_left_y - mouth_height // 2

        resize_and_apply_filter(red_thick_mouth_filter, mouth_x, mouth_y, int(mouth_width * 1.2), int(mouth_height * 1.2), image)
