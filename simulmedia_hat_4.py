...
def apply_unicorn_hat_filter(image, face_landmarks):

    image_height, image_width, _ = image.shape

    unicorn_filter = cv2.imread("C:/Users/PC/image/hat4.png", cv2.IMREAD_UNCHANGED)
    ...
    hat_width = int(unicorn_filter.shape[1] * (hat_height / unicorn_filter.shape[0]))  # 비율에 맞게 모자 너비 계산

    # 모자를 이마 위쪽으로 배치
    resize_and_apply_filter(unicorn_filter, forehead_x - hat_width // 2, max(0, forehead_y - hat_height), hat_width, hat_height, image)
