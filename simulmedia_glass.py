...
def apply_eye_filter(image, face_landmarks):

    image_height, image_width, _ = image.shape

    glasses_filter = cv2.imread("C:/Users/PC/image/glasses.png", cv2.IMREAD_UNCHANGED)  # 알파 채널 포함된 이미지
    ...
    glasses_height = int(glasses_filter.shape[0] * (glasses_width / glasses_filter.shape[1]))  # 원래 비율에 맞춰 높이 계산

    # 안경 위치를 약간 위쪽으로 조정하여 자연스럽게 적용
    resize_and_apply_filter(glasses_filter, left_eye_outer_x - int(glasses_width * 0.17),
                            int(middle_eye_y - glasses_height // 2.2), glasses_width, glasses_height)
