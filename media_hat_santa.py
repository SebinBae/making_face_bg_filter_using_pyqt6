...
def apply_hat_santa_filter(image, face_landmarks):

    image_height, image_width, _ = image.shape

    hat_filter = cv2.imread("C:/Users/PC/image/hat_santa.png", cv2.IMREAD_UNCHANGED)
    ...
    # 모자 너비와 높이를 적절히 설정
    hat_height = int(image_height * 0.45)  # 얼굴의 45% 높이로 모자 설정 (세로 크기 증가)
    hat_width = int(hat_filter.shape[1] * (hat_height / hat_filter.shape[0]) * 1.1)  # 원래 비율에 맞게 모자 너비 계산
    ...
    # 모자의 위치를 이마 위쪽으로 배치 (머리와의 거리 차이 계산 포함)
    x_offset = forehead_x - hat_width // 2 + int(hat_width * 0.05)
    y_offset = max(0, int(forehead_y - hat_height // 2.2) + int(hat_height * 0.1))
