...
def apply_hat_unicorn_filter(image, face_landmarks):

    image_height, image_width, _ = image.shape

    hat_filter = cv2.imread("C:/Users/PC/image/hat_unicorn.png", cv2.IMREAD_UNCHANGED)
    ...
    hat_width = int(hat_filter.shape[1] * (hat_height / hat_filter.shape[0]))  # 원래 비율에 맞게 모자 너비 계산
    ...
    y_offset = max(0, forehead_y - hat_height)
