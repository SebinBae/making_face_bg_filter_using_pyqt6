...
def apply_hat_filter(image, face_landmarks):
    image_height, image_width, _ = image.shape

    hat_filter = cv2.imread("C:/Users/PC/image/hat.png", cv2.IMREAD_UNCHANGED)
    ...
    hat_width = int(hat_filter.shape[1] * (hat_height / hat_filter.shape[0]))  # 비율에 맞게 모자 너비 계산

    # 모자를 이마 위쪽으로 배치
    resize_and_apply_filter(hat_filter, forehead_x - hat_width // 2, max(0, forehead_y - hat_height),
                            hat_width,
                            hat_height, image)
