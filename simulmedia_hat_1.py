...
def apply_hat1_filter(image, face_landmarks):

    image_height, image_width, _ = image.shape

    hat_filter = cv2.imread("C:/Users/PC/image/hat1.png", cv2.IMREAD_UNCHANGED)
    ...
    hat_width = int(hat_filter.shape[1] * (hat_height / hat_filter.shape[0]) * 1.1)  # 원래 비율에 맞게 모자 너비 계산, 1.3배 넓게

    # 모자를 이마 위쪽으로 배치 (조금 더 내려오도록 조정)
    resize_and_apply_filter(hat_filter, forehead_x - hat_width // 2, max(0, int(forehead_y - hat_height // 1.5)), hat_width, hat_height, image)
