...
def apply_mouth_mask_filter(image, face_landmarks):

    image_height, image_width, _ = image.shape

    mouth_filter = cv2.imread("C:/Users/PC/image/mouth_mask.png", cv2.IMREAD_UNCHANGED)
    ...
    # 필터 크기 계산
    mouth_width = (mouth_right_x - mouth_left_x) * 4.4  # 가로 크기를 4.4배 증가
    if mouth_width > 0:
        mouth_height = int(mouth_filter.shape[0] * (mouth_width / mouth_filter.shape[1]))
        mouth_x = int((mouth_left_x + mouth_right_x) / 2 - mouth_width / 2) + 5  # 중앙으로 위치 조정
        mouth_y = int((mouth_left_y + mouth_right_y) / 2 - mouth_height / 2) - 7
